...
function renderLexicalNode(node: LexicalNode, key: string): ReactNode {
  ...
  switch (node.type) {
     ...
     * INLINE IMAGE / UPLOAD
     *
     * Gambar yang dimasukkan dari tombol Upload di dalam
     * Payload Rich Text / Lexical akan menghasilkan node
     * dengan type "upload".
     *
     * Media hasil relationship Payload berada di node.value.
     */
    case 'upload': {
      const media =
      ...
      /*
       * Jika Payload hanya mengembalikan ID media dan bukan
       * object media lengkap, URL belum tersedia.
       *
       * Dalam kondisi tersebut kita tidak merender apapun.
       * Jika ini terjadi, query Payload perlu dinaikkan depth-nya.
       */
      if (!media) {
        return null
      }
  ...
  return (
    <div className="article-body unified-article-body">
      {article.body.map((block, index) => {
         ...
         */
        if (
          blockData.blockType === 'articleImage'
        ) {
          const media =
         ...
         */
        if (
          blockData.blockType === 'youtubeVideo'
        ) {
          const embedUrl =
            getYouTubeEmbedUrl(blockData.url)
